...
public class SensorGraphView extends View {
    ...
    private ArrayList<Float>[] normalisedDataPoints;//this will store [dimensions of sensor values][number of data points] array
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < this.normalisedDataPoints.length; ++i) {
            ...
            for (Float dataPoint : this.normalisedDataPoints[i]) {
                ...
                float y = height - (height * dataPoint); // y value is the normalized height of that point

                canvas.drawCircle(currentX, y, dataPointsAccuracy[i].get(index), rectPaints[i]);// point radius proportional to accuracy
